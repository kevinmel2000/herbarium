@extends('verified_herba.lichenherba.base')
...
<section class="content">
    <div class="box">
        <div class="box-header">
            <div class="row">
                <div class="col-sm-8">
                    <h3 class="box-title">List of Verification Lichen Herbarium (Verified)</h3>
                </div>
            </div>
        </div>
  <!-- /.box-header -->
                                        ...
                                        @if($herba->type== 4)
                                            @if($herba->status == 2)
                                                <tr role="row" class="odd">
                                                    ...
                                                    <td>
                                                        <form class="row" method="POST" action="{{ route('lichenherba.destroy', ['id_herba' => $herba->id_herbarium]) }}" onsubmit = "return confirm('Are you sure?')"  style="text-align: center">
                                                            <input type="hidden" name="_method" value="DELETE">
                                                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                            <a href="{{ route('lichenherba.show', ['id_herba' => $herba->id_herbarium]) }}" class="btn btn-success">View</a>
                                                            <a href="{{ route('lichenherba.edit', ['id_herba' => $herba->id_herbarium]) }}" class="btn btn-warning">Edit</a>
